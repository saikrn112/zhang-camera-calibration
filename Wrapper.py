...
def convert_A_to_vector(A):
    return np.array([A[0,0],A[1,1],A[0,2],A[1,2]])
...
def get_camera_intrinsic_from_b(b):
    ...
    A  = convert_A_vector_to_matrix([alpha,beta,u0,v0])
    A[0,1] = gamma
    return A, lamda
...
def compute_residuals(x,imgs_corners,world_corners,transformations):
    ...
    A = convert_A_vector_to_matrix(x[0:4]) 
    k1 = x[4]
    k2 = x[5]

    #transformation_params = x[6:] # N*M*nP
...
def inverse_warp(img,A,k1,k2):
    ...
    alpha,beta,u0,v0 = convert_A_to_vector(A)
...
def main(args):
    ...
    A_final = convert_A_vector_to_matrix(result.x[0:4])
    A_final[0,1] = A_estimate[0,1]
    k1 = result.x[4]
    k2 = result.x[5]
